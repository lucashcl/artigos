...
export async function queue(
   ...
   await Promise.allSettled(
      batch.messages.map(async (message) => {
         console.log(
            `Processing queue item: ${message.body.id} - ${message.body.url}`,
         );
      ...
      `);
         ...
         const result = await env.summaries_db
            .prepare(
               `
         UPDATE queue_items
      ...
      `,
         ...
         if (result.meta.changes === 0) {
            console.log(
               `Queue item ${id} is already being processed by another worker.`,
            );
            message.ack();
            return;
         }
         ...
         try {
            ...
            await env.summaries_db.batch([
               ...
               preparedUpdateStatus.bind("completed", id),
            ]);
         } catch (error) {
            console.error(
               `Error processing queue item ${id}:`,
               error,
            ...
            await preparedUpdateStatus.bind("failed", id).run();
         }
      }),
   );
}
